...
import { Award, BookOpen, TrendingUp, X } from "lucide-react";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (historyRes.data)
          setPracticeHistory(historyRes.data.practice_sessions || []);
        if (coursesRes.data)
          setCourses(coursesRes.data.map(transformCourseForUI));
        setError(null);
      } catch (e) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 shadow-sm border flex flex-col items-center">
            ...
            <div className="text-2xl font-bold">
              {stats.statistics?.total_sessions ?? 0}
            </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 shadow-sm border flex flex-col items-center">
            ...
            <div className="text-2xl font-bold">
              {stats.statistics?.average_score ?? 0}
            </div>
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {courses.map((course) => (
              <div
                key={course.id}
                className="bg-white dark:bg-zinc-900 rounded-xl p-4 border flex flex-col"
              >
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  >
                    {unenrollLoading[course.id] ? "..." : "Unenroll"}
                  </button>
                </div>
              </div>
            ))}
